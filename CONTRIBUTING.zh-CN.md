...
`fluoh package port` 会保持上游分支干净，把克隆来源保留为 `upstream`，创建 `ohos/3.35/camera` 这类 FlutterOH Package 分支，把 `origin` 设置为 package 仓库最终推送位置，并配置所选 FlutterOH SDK 环境。默认仓库 URL 会根据 package 名称推导：

```sh
https://github.com/FlutterOH/<package>.git
```

如果某个 package 需要推送到独立 FlutterOH package 仓库，移植时使用 `--repository` 指定：
...
`fluoh package port` 会暂存生成的 `FLUOH.md`、`fluoh.yaml` 和 `doc/fluoh/<package>/spec.md`。如果选中的 package 已有 Flutter example，它还会为该 example 新增 OHOS 平台并固定到选中的 SDK，但不会创建初始提交。维护者可以继续完成 FlutterOH 支持实现，最后用维护者自己的 Git 身份一起提交。运行任何要求干净工作区的命令前需要先提交：

```sh
git commit -m "feat(package): initialize FlutterOH package"
```

`fluoh package upstream check` 和 `fluoh package upstream sync` 只用于 `origin.kind: ported`。`upstream sync` 会拉取 upstream 分支和 tags，快进同步 Package `fluoh.yaml` 记录的上游分支，把选中的 Package 目标提交合入当前 `ohos/<sdkLine>/<package>` 分支，并刷新 `fluoh.yaml` 中的 upstream 元数据。默认目标是该 Package 最新有效 upstream release tag；需要指定且不低于当前 upstream version 的 Package 版本时传 `--upstream-version <version>`。`upstream sync` 会拒绝 upstream 降级；这种情况应使用 `fluoh package version --status broken` 标记当前支持分支为 broken。sync 后先按新的 upstream version 和 commit review `doc/fluoh/<package>/spec.md`，再继续实现。新的 FlutterOH 支持完成前保持 Package release version 不变。
...
- 缺失或未填写当前版本的 `FLUOH.md` FlutterOH Release History 发布说明会提示 warning，但不阻塞 release。
...
FlutterOH package 仓库的 release 命令不得直接写入 source 元数据。首次发布的 Package 分支用 `fluoh source register` 注册，后续发布记录通过 `fluoh source sync` 从已发布 package 仓库生成；路由、advisory 和 maintenance 元数据直接编辑 Source 和 Manifest YAML。FlutterOH/source pull request 应运行 `fluoh source check <source-pr-url> --json`，或在本地 checkout 中运行 `fluoh source check . --json`；定时 release-gate 任务可以使用 `fluoh source check . --all --json`。source check 输出只作为技术 check/comment 信号；approval 和 merge 决策仍由维护者手动完成。
