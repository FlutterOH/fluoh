...
List<String> _multiAdaptationCommandFlowLines() {
  return [
    ...
    '4. OHOS loop: run `fluoh run ohos --package <name> --auto-emulator --json --trace-dir <trace-dir>`, or add `--device-id <id>` for a connected OHOS target. This prepares debug signing, launches through `flutter run`, captures run/session evidence, and executes `example/integration_test/` on the selected target when present. After a successful mobile run, capture a post-launch screenshot or equivalent UI-state artifact with a `fluoh drive` scenario and verify the demo page is usable before moving to broader automation; fix blank, splash-only, hidden, or otherwise abnormal demo pages first. If no local target can be started, run `fluoh build ohos --package <name> --auto-sign --json --trace-dir <trace-dir>` as build-only evidence and record the environment blocker.',
    '5. Existing-platform loop: for Android, run `fluoh doctor --platform android --json --strict`, then `fluoh run android --package <name> --auto-emulator --json` when `example/android` exists. For iOS, run `fluoh doctor --platform ios --json --strict`, then `fluoh run ios --package <name> --auto-emulator --json` when `example/ios` exists. For macOS, run `fluoh doctor --platform macos --json --strict`, then `fluoh run macos --package <name> --json` when `example/macos` exists. For Web, run `fluoh doctor --platform web --json --strict`, then `fluoh run web --package <name> --json` when `example/web` exists. For Linux and Windows, run the matching doctor command and `fluoh build linux|windows --package <name> --json` when those example directories exist. After successful Android or iOS runs, capture post-launch screenshot or UI-state evidence and repair abnormal demo pages before continuing. Add or run `integration_test/` for tappable workflows; otherwise run an AI-assisted interaction scenario or manual-assisted fallback and record the reviewer, target, result, blocker, and tool-readable evidence beyond launch-only session state.',
...
List<String> _multiPlatformVerificationLines() {
  return [
    ...
    '- For run smoke checks, `fluoh run` records platform launch evidence and can write a `flutterRunSession` file with `--session-file <path>`. Platform runs launch through `flutter run`, capture output for `--log-duration`, capture `details.vmServiceUri` when Flutter prints a VM Service or debug service URI, send `d` to detach while leaving the app running, save project-local output under `.fluoh/cache/package-runs`, and report runtime failures through JSON diagnostics. AI agents can run `python3 <skill-dir>/scripts/inspect_session.py <session-file> --wait 30 --expect-platform <platform>` to wait for launch/session evidence, then use `fluoh attach <platform> --session-file <path>` for Flutter debug inspection when needed and read `workflowEvidence.notCollectedEvidenceKinds`; for mobile platforms, `postLaunchScreenshot` and `visualPageReadiness` mean the next step must be a `fluoh drive` scenario that captures a screenshot or equivalent UI-state artifact and asserts the demo page is ready before broader automation.',
