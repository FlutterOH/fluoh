...
List<String> _multiAdaptationCommandFlowLines() {
  return [
    ...
    '2. Baseline gates: review `adaptationProfile.officialDocTopics`, record the official OHOS/OpenHarmony or vendor SDK documentation basis, then run `fluoh deps get`, `fluoh doctor --project --json --strict`, `fluoh flutter analyze`, and existing package or example tests before adding OHOS code. Project warnings mean repository fixes; environment warnings mean local toolchain or Source fixes.',
    ...
    '4. OHOS loop: run `fluoh run ohos --package <name> --auto-emulator --json --trace-dir <trace-dir>`, or add `--device-id <id>` for a connected OHOS target. This prepares debug signing, launches through `flutter run`, captures run/session evidence, attempts `details.postLaunchScreenshot`, and executes `example/integration_test/` on the selected target when present. After a successful mobile run, inspect that screenshot evidence or collect equivalent UI-state evidence with a `fluoh drive` scenario, then verify the demo page is usable before moving to broader automation; fix blank, splash-only, hidden, or otherwise abnormal demo pages first. If no local target can be started, run `fluoh build ohos --package <name> --auto-sign --json --trace-dir <trace-dir>` as build-only evidence and record the environment blocker.',
    '5. Existing-platform loop: for Android, run `fluoh doctor --platform android --json --strict`, then `fluoh run android --package <name> --auto-emulator --json` when `example/android` exists. For iOS, run `fluoh doctor --platform ios --json --strict`, then `fluoh run ios --package <name> --auto-emulator --json` when `example/ios` exists. For macOS, run `fluoh doctor --platform macos --json --strict`, then `fluoh run macos --package <name> --json` when `example/macos` exists. For Web, run `fluoh doctor --platform web --json --strict`, then `fluoh run web --package <name> --json` when `example/web` exists. For Linux and Windows, run the matching doctor command and `fluoh build linux|windows --package <name> --json` when those example directories exist. After successful Android or iOS runs, inspect `details.postLaunchScreenshot` or collect UI-state evidence and repair abnormal demo pages before continuing. Add or run `integration_test/` for tappable workflows; otherwise run an AI-assisted interaction scenario or manual-assisted fallback and record the reviewer, target, result, blocker, and tool-readable evidence beyond launch-only session state.',
    ...
    '7. Implementation checkpoint: once implementation, OHOS evidence, and applicable existing-platform regression checks are clean, or only local environment blockers remain recorded with diagnostic evidence, create a local implementation checkpoint commit. This clean worktree is required before `fluoh package version`, `fluoh package sync`, and `fluoh package check`.',
...
List<String> _multiPlatformVerificationLines() {
  return [
    ...
    '- During OHOS grant-path integration tests, pass `--ohos-permission-dialog-policy allow` only when automatic allow preserves test intent; the integration step then records `details.systemPermissionDialogs`. Treat that as prompt-handling evidence, and keep deny/error paths under `integration_test` or `fluoh drive` control so automatic allow does not mask those behaviors.',
    ...
    '- For run smoke checks, `fluoh run` records platform launch evidence and can write a `flutterRunSession` file with `--session-file <path>`. Platform runs launch through `flutter run`, capture output for `--log-duration`, capture `details.vmServiceUri` when Flutter prints a VM Service or debug service URI, send `d` to detach while leaving the app running, save project-local output under `.fluoh/cache/package-runs`, attempt `details.postLaunchScreenshot` for OHOS, Android, and iOS, and report runtime failures through JSON diagnostics. AI agents can run `python3 <skill-dir>/scripts/inspect_session.py <session-file> --wait 30 --expect-platform <platform>` to wait for launch/session evidence, then use `fluoh attach <platform> --session-file <path>` for Flutter debug inspection when needed and read `workflowEvidence.notCollectedEvidenceKinds`; when `postLaunchScreenshot` is still not collected, use a `fluoh drive` scenario to capture a screenshot or equivalent UI-state artifact. `visualPageReadiness` means a tool-readable assertion must confirm the demo page is ready before broader automation.',
    '- Run smoke is not enough for workflows that need UI taps, permission prompts, files, camera, location, media playback, deep links, or external apps. Cover those flows with `integration_test/`, or run an AI-assisted functional scenario and record exact steps, expected result, actual result, device or simulator id, Flutter debug/widget/semantic/log evidence, and post-launch screenshots or UI-state captures.',
    '- Official OHOS/OpenHarmony or vendor SDK documentation basis is required for platform plugin behavior. If a required document is unavailable, keep adapting but record the unavailable source, fallback evidence, and implementation impact in the report before any `ready` recommendation.',
    '- Store scenario notes under `.fluoh/scenarios/<package>/<platform>-<name>.md` when the interaction is not already encoded as `integration_test`. The AI driver follows the scenario, operates the target with available device or UI tools, and records pass/fail evidence in the report. The scenario must capture post-launch visual or UI-state evidence and then validate behavior with Flutter debug or VM service output, widget/component tree state, semantics, stable visible text, command JSON, or logs rather than relying on screenshot appearance alone.',
    ...
    '- OHOS `fluoh run` executes `integration_test/` when present. For required flows that are not yet encoded as integration tests, add tests first; use AI-assisted evidence for device-operable flows. Use manual-assisted mode only when the available device tools cannot drive a specific step, and do not mark it ready unless tool-readable evidence beyond launch-only session state verifies the flow.',
...
List<String> _singlePlatformVerificationLines(
  ...
  return [
    ...
    '- During OHOS grant-path integration tests, pass `--ohos-permission-dialog-policy allow` only when automatic allow preserves test intent; the integration step then records `details.systemPermissionDialogs`. Treat that as prompt-handling evidence, and keep deny/error paths under `${package.examplePath}/integration_test/` or `fluoh drive` control so automatic allow does not mask those behaviors.',
    ...
    '- For run smoke checks, `fluoh run` records platform launch evidence and can write a `flutterRunSession` file with `--session-file <path>`. Platform runs launch through `flutter run`, capture output for `--log-duration`, capture `details.vmServiceUri` when Flutter prints a VM Service or debug service URI, send `d` to detach while leaving the app running, save project-local output under `.fluoh/cache/package-runs`, attempt `details.postLaunchScreenshot` for OHOS, Android, and iOS, and report runtime failures through JSON diagnostics. AI agents can run `python3 <skill-dir>/scripts/inspect_session.py <session-file> --wait 30 --expect-platform <platform>` to wait for launch/session evidence, then use `fluoh attach <platform> --session-file <path>` for Flutter debug inspection when needed and read `workflowEvidence.notCollectedEvidenceKinds`; when `postLaunchScreenshot` is still not collected, use a `fluoh drive` scenario to capture a screenshot or equivalent UI-state artifact. `visualPageReadiness` means a tool-readable assertion must confirm the demo page is ready before broader automation.',
    '- Run smoke is not enough for workflows that need UI taps, permission prompts, files, camera, location, media playback, deep links, or external apps. Cover those flows with `${package.examplePath}/integration_test/`, or run an AI-assisted functional scenario and record exact steps, expected result, actual result, device or simulator id, Flutter debug/widget/semantic/log evidence, and post-launch screenshots or UI-state captures.',
    '- Official OHOS/OpenHarmony or vendor SDK documentation basis is required for platform plugin behavior. If a required document is unavailable, keep adapting but record the unavailable source, fallback evidence, and implementation impact in the report before any `ready` recommendation.',
    '- Store scenario notes under `.fluoh/scenarios/${package.name}/<platform>-<name>.md` when the interaction is not already encoded as `integration_test`. The AI driver follows the scenario, operates the target with available device or UI tools, and records pass/fail evidence in the report. The scenario must capture post-launch visual or UI-state evidence and then validate behavior with Flutter debug or VM service output, widget/component tree state, semantics, stable visible text, command JSON, or logs rather than relying on screenshot appearance alone.',
    '- Consider every applicable interaction class before release: permission grant and denial, file or media picker, camera or microphone capture, location and sensors, maps, media playback or recording, deep links and external app callbacks, background or lifecycle behavior, multi-step forms, and negative/error paths. If none apply, write `No interaction required: <reason>` in the report.',
...
List<String> _completionReportLines() {
  return [
    ...
    '- The report must include: package name, upstream version, FlutterOH SDK version, implementation summary, changed files, public API changes if any, official OHOS/OpenHarmony or vendor SDK documentation basis, permissions and OHOS config changes, commands run with exit results and relevant output tails, final `fluoh run <platform> --json` or build outcome, post-launch screenshot or UI-state evidence for each successful mobile run, a platform matrix for OHOS/Android/iOS/macOS/Linux/Web/Windows with build, run, integration-test, device/simulator/host/browser, and skip-blocker fields, real `fluoh drive --json` evidence, automation coverage gates from dry-run or real drive JSON, functional interaction evidence for required scenarios, independent reviewer feedback packet, findings, repair statuses, validation evidence, signing mode, generated HAP paths, hilog path when present, remaining risks, and release recommendation.',
...
String packageImplementationGuideContent({
  ...
  return [
    ...
    '10. Run final `fluoh verify --package <name>`, create the ignored `.fluoh/reports/<name>/report-<timestamp>.md`, run `$_reportCheckCommand`, then start a separate host reviewer agent for read-only review. Record the feedback packet and repair reviewer findings with validation evidence before running `fluoh package check --package <name> --report <report-path>` while the tracked worktree is clean.',
    '',
    '## Adaptation Workflow',
    '',
    ...
    '- `FLUOH_CHANGELOG.md`, `fluoh.yaml`, package status, release version, and independent reviewer feedback packet has no open blocker/high/medium items for `fluoh package check` and `fluoh package release`.',
    '',
...
String _singlePackageImplementationGuideContent({
  ...
  return [
    ...
    '10. Run final `${package.verifyCommand}`, create the ignored `.fluoh/reports/${package.name}/report-<timestamp>.md`, run `$_reportCheckCommand`, then start a separate host reviewer agent for read-only review. Record the feedback packet and repair reviewer findings with validation evidence before running `${package.releaseCheckCommand} --report <report-path>` while the tracked worktree is clean.',
    '',
    '## Adaptation Workflow',
    '',
    '1. Inventory: read `fluoh.yaml` to confirm SDK version, package path, upstream version, current release status, and example location.',
    ...
    '- `FLUOH_CHANGELOG.md`, `fluoh.yaml`, package status, release version, and independent reviewer feedback packet has no open blocker/high/medium items for `${package.releaseCommand}`.',
    '',
    ..._completionReportLines(),
