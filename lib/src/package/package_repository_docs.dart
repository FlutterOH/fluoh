...
class PackageRepositoryDocPackage {
  ...
  String get linuxRunCommand => packagePath == '.'
      ? 'fluoh run --platform linux --json'
      : 'fluoh run --platform linux --package $name --json';
  ...
  String get windowsRunCommand => packagePath == '.'
      ? 'fluoh run --platform windows --json'
      : 'fluoh run --platform windows --package $name --json';

  /// Web example build command.
  String get webBuildCommand => packagePath == '.'
      ? 'fluoh build --platform web --json'
...
List<String> _multiAdaptationCommandFlowLines() {
  return [
    ...
    '4. OHOS loop: run `fluoh run --platform ohos --package <name> --auto-emulator --json --trace-dir <trace-dir>`, or add `--device <id>` for a connected hdc target. This proves build, signing, install, launch, and hilog diagnostics; then complete required example functional interactions with AI-assisted automation when possible and record evidence. If no local target can be started, run `fluoh build --platform ohos --package <name> --auto-sign --json --trace-dir <trace-dir>` as build-only evidence and record the environment blocker.',
    '5. Existing-platform loop: for Android, run `fluoh doctor --platform android --json --strict`, then `fluoh run --platform android --package <name> --auto-emulator --json` when `example/android` exists. For iOS, run `fluoh doctor --platform ios --json --strict`, then `fluoh run --platform ios --package <name> --auto-emulator --json` when `example/ios` exists. For macOS, run `fluoh doctor --platform macos --json --strict`, then `fluoh run --platform macos --package <name> --json` when `example/macos` exists. For Web, run `fluoh doctor --platform web --json --strict`, then `fluoh run --platform web --package <name> --device web-server --json` when `example/web` exists. For Linux and Windows, run the matching doctor command and `fluoh build --platform linux|windows --package <name> --json` when those example directories exist. Add or run `integration_test/` for tappable workflows; otherwise run an AI-assisted interaction scenario and record evidence.',
    ...
    '9. Final report and release gate: rerun final `fluoh verify --package <name>`, create `.fluoh/reports/<name>/ai-report-<name>-YYYYMMDD-HHMMSS.md` with command results, platform matrix, interaction evidence, diagnostics, Fluoh Feedback entries from trace manifests or an explicit `No fluoh feedback: <reason>` statement, signing mode, logs, risks, and release recommendation, create `.fluoh/reports/<scope>/summary-<scope>-YYYYMMDD-HHMMSS.md` for multi-package monorepo work, run `$_reportCheckCommand`, then run `fluoh package check --package <name> --report .fluoh/reports/<name>/ai-report-<name>-YYYYMMDD-HHMMSS.md`. `.fluoh/` is ignored local state and must not be committed, so the worktree should remain clean for package check. Add `--require-ohos-run` when an OHOS target was available and the handoff must prove a passed real run. Maintainers can still use baseline `fluoh package check --package <name>` after their own manual verification. Release only after maintainer approval with `fluoh package release --package <name>`.',
    '',
  ];
}
...
List<String> _singleAdaptationCommandFlowLines(
  ...
  return [
    ...
    '9. Final report and release gate: rerun final `${package.verifyCommand}`, create `.fluoh/reports/${package.name}/ai-report-${package.name}-YYYYMMDD-HHMMSS.md` with command results, platform matrix, interaction evidence, diagnostics, Fluoh Feedback entries from trace manifests or an explicit `No fluoh feedback: <reason>` statement, signing mode, logs, risks, and release recommendation, run `$_reportCheckCommand`, then run `${package.releaseCheckCommand} --report .fluoh/reports/${package.name}/ai-report-${package.name}-YYYYMMDD-HHMMSS.md`. `.fluoh/` is ignored local state and must not be committed, so the worktree should remain clean for package check. Add `--require-ohos-run` when an OHOS target was available and the handoff must prove a passed real run. Maintainers can still use baseline `${package.releaseCheckCommand}` after their own manual verification. Release only after maintainer approval with `${package.releaseCommand}`.',
    '',
  ];
}
...
List<String> _multiPlatformVerificationLines() {
  return [
    ...
    '',
    '- OHOS: run `fluoh run --platform ohos --package <name> --auto-emulator --json`, or add `--device <id>` for a connected hdc target. This is the authoritative OHOS build, signing, install, launch, hilog, and diagnostics loop.',
    '- Android: when `example/android` exists, first run `fluoh doctor --platform android --json --strict`, then run `fluoh run --platform android --package <name> --auto-emulator --json` to prefer a local emulator, or add `--device <id>` only when no emulator is available. If `integration_test/` exists, the command runs it on the selected target after the smoke run.',
    '- iOS: when `example/ios` exists, first run `fluoh doctor --platform ios --json --strict`, then run `fluoh run --platform ios --package <name> --auto-emulator --json` to prefer a simulator, or add `--device <id>` only when no simulator is available. If `integration_test/` exists, the command runs it on the selected target after the smoke run. Do not commit team-specific signing state.',
    ...
    '- Linux: when `example/linux` exists, first run `fluoh doctor --platform linux --json --strict`, then run `fluoh build --platform linux --package <name> --json` on a Linux host. If a Linux run smoke is required, use `fluoh run --platform linux --package <name> --json` on that host.',
    '- Web: when `example/web` exists, first run `fluoh doctor --platform web --json --strict`, then run `fluoh run --platform web --package <name> --device web-server --json` for served web smoke evidence. Add `fluoh build --platform web --package <name> --json` when a separate web compile check is needed; use Chrome from `fluoh devices --platform web` only when browser-specific evidence is required.',
    '- Windows: when `example/windows` exists, first run `fluoh doctor --platform windows --json --strict`, then run `fluoh build --platform windows --package <name> --json` on a Windows host. If a Windows run smoke is required, use `fluoh run --platform windows --package <name> --json` on that host.',
...
List<String> _singlePlatformVerificationLines(
  ...
  return [
    ...
    '- Release recommendation `ready` requires passing build and run or integration evidence for every existing supported platform directory. A missing local Android, Xcode, macOS/Linux/Web/Windows host or browser, device, or simulator environment is a maintainer-decision blocker, not a ready release.',
    '- Record each platform as `not present`, `passed`, `failed`, or `skipped with blocker` in the `.fluoh/reports/${package.name}/ai-report-${package.name}-...md` platform matrix. Include command, device id or simulator id, exit result, and relevant output tail.',
    '',
  ];
}
...
List<String> _diagnosticsRoutingLines() {
  return [
    ...
    '- `android.device_ambiguous`, `ios.device_ambiguous`, `macos.device_ambiguous`, `linux.device_ambiguous`, `web.device_ambiguous`, `windows.device_ambiguous`: pick one target and rerun with `--device <id>`.',
    ...
    '- `web.integration_target_unsupported`: rerun Web with a browser target such as `--device chrome`; `web-server` is only valid for served smoke checks.',
    ...
    '- `ohos.device_missing`: rerun the OHOS run command with `--auto-emulator`, start an OpenHarmony emulator, or connect a device and add `--device <id>`.',
...
List<String> _completionReportLines() {
  return [
    '## Completion Report',
    '',
    '- Before final response, create a timestamped report in the repository root under `.fluoh/reports/<package-or-scope>/ai-report-<package-or-scope>-YYYYMMDD-HHMMSS.md`. For multi-package monorepos, also create `.fluoh/reports/<scope>/summary-<scope>-YYYYMMDD-HHMMSS.md`. Use local time, 24-hour time, and a timestamp precise to seconds, for example `.fluoh/reports/camera/ai-report-camera-20260526-153045.md`. Create `.fluoh/reports/<package-or-scope>/` if needed; it is local state and must not be committed.',
    '- The report must include: package name, upstream version, FlutterOH SDK version, implementation summary, changed files, public API changes if any, permissions and OHOS config changes, commands run with exit results and relevant output tails, final `fluoh run --platform <platform> --json` or build outcome, a platform matrix for OHOS/Android/iOS/macOS/Linux/Web/Windows with build, run, integration-test, device/simulator/host/browser, and skip-blocker fields, functional interaction evidence for required scenarios, signing mode, generated HAP paths, hilog path when present, remaining risks, and release recommendation.',
...
String packageImplementationGuideContent({
  ...
  return [
    ...
    '5. Run the full automated OHOS loop for each package example: `fluoh run --platform ohos --package <name> --auto-emulator --json`. Add `--device <id>` for an already connected hdc target. Use `fluoh build --platform ohos --package <name> --auto-sign --json` as a build-only fallback only when no local target can be started. JSON `nextCommand` and `diagnostics` give the next failure category. Complete and record required OHOS example functional interactions separately because the run command does not click through pages.',
    '6. Run existing-platform regression checks after shared or example changes when their platform directories exist: run `fluoh doctor --project --json --strict`, then use `fluoh run --platform android --package <name> --auto-emulator --json` for Android, `fluoh run --platform ios --package <name> --auto-emulator --json` for iOS, `fluoh run --platform macos --package <name> --json` for macOS, `fluoh run --platform web --package <name> --device web-server --json` for Web, and `fluoh build --platform linux|windows --package <name> --json` on matching Linux/Windows hosts. Add `--device <id>` only when no emulator or simulator is available. iOS builds use `--no-codesign`; document unavailable local Android/Xcode/macOS/Linux/Web/Windows host or browser toolchains instead of guessing. Add `integration_test/` or AI-assisted interaction evidence for flows beyond launch smoke.',
    ...
    '7. Build and run OHOS: use `fluoh run --platform ohos --package <name> --auto-emulator --json` to build, auto-sign, install, launch, capture hilog, and classify failures. Then perform required example functional interactions on the emulator or device and record evidence. Fix permission, `reason`, `usedScene`, ArkTS, install, launch, runtime, or interaction diagnostics before release.',
    '8. Check existing platforms: follow the Platform Verification Matrix. Run `fluoh run --platform android --package <name> --auto-emulator --json` when an Android example exists, `fluoh run --platform ios --package <name> --auto-emulator --json` when an iOS example exists, `fluoh run --platform macos --package <name> --json` when a macOS example exists, `fluoh run --platform web --package <name> --device web-server --json` when a Web example exists, and `fluoh build --platform linux|windows --package <name> --json` when Linux or Windows examples exist on matching hosts. Add `--device <id>` only when no emulator or simulator is available. Use `integration_test/` or AI-assisted interaction evidence for scenario coverage beyond launch smoke. Record exact skip reasons for unavailable local toolchains.',
...
String _singlePackageImplementationGuideContent({
  ...
  return [
    ...
    '10. Run final `${package.verifyCommand}`, create the ignored `.fluoh/reports/${package.name}/ai-report-${package.name}-...md`, run `$_reportCheckCommand`, then run `${package.releaseCheckCommand} --report <report-path>` while the tracked worktree is clean.',
    '',
    '## Adaptation Workflow',
    '',
    '1. Inventory: read `fluoh.yaml` to confirm SDK version, package path, upstream version, current release status, and example location.',
    ...
    '8. Check existing platforms: follow the Platform Verification Matrix. Run `${package.androidEmulatorRunCommand}` when an Android example exists, `${package.iosSimulatorRunCommand}` when an iOS example exists, `${package.macosRunCommand}` when a macOS example exists, `${package.webRunCommand}` when a Web example exists, `${package.linuxBuildCommand}` when a Linux example exists, and `${package.windowsBuildCommand}` when a Windows example exists, or add `--device <id>` for already connected mobile targets. Use `${package.examplePath}/integration_test/` or AI-assisted interaction evidence for scenario coverage beyond launch smoke. Record exact skip reasons for unavailable local toolchains.',
    ...
    '10. Finish: update `FLUOH_CHANGELOG.md`, commit the release metadata checkpoint, run final `${package.verifyCommand}`, create the ignored `.fluoh/reports/${package.name}/ai-report-${package.name}-...md`, run `$_reportCheckCommand`, then run `${package.releaseCheckCommand} --report <report-path>`. Use `${package.releaseCommand}` only after maintainer approval.',
    '',
    '## Release Readiness',
    '',
    '- Public Dart APIs remain compatible with upstream unless upstream changed them.',
