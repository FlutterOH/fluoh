...
String packageAgentsInstructionsContent({
  ...
  return [
    ...
    '- Prefer the full automated OHOS loop when a local DevEco emulator or device is available: `fluoh run --platform ohos --package <name> --json`. Add `--device <id>` for an already connected hdc target.',
    '- After changing shared Dart code, dependencies, examples, or platform-channel contracts, run existing-platform regression checks when the example has those platforms: `fluoh run --platform android --package <name> --json` for Android and `fluoh run --platform ios --package <name> --json` for iOS. Add `--emulator <name>` to start a local emulator or simulator, or `--device <id>` to select a connected target. iOS builds use `--no-codesign` automatically.',
    '- Run `fluoh doctor --project --json --strict` when Android SDK, Xcode, DevEco Studio, signing tools, hdc, emulator, current project, selected SDK, fluoh installation, or source snapshot health is unclear; fix warnings before editing package logic.',
    ...
    '2. Baseline: work one package at a time. Run `fluoh deps get`, `fluoh flutter analyze`, existing package tests, `fluoh doctor --project --json --strict`. Include Android/iOS run commands when `example/android` or `example/ios` exists and the local toolchain is available; fix non-OHOS regressions first.',
    ...
    '8. Verify existing platforms: after OHOS changes, follow the Platform Verification Matrix for each existing platform directory. Build-only evidence is not enough for release readiness; use `fluoh doctor --platform android --json --strict` or `fluoh doctor --platform ios --json --strict` to check native tooling, then use `fluoh run --platform android --package <name> --json` or `fluoh run --platform ios --package <name> --json` so Android and iOS examples are launched and integration tests run when present. If a local Android, Xcode, device, or simulator environment is unavailable, record the exact skipped command and reason in the report.',
...
String _singlePackageAgentsInstructionsContent({
  ...
  return [
    ...
    '- Treat example apps as real verification surfaces: every important workflow should provide a visible operation, expected result, pass/fail status, and failure hint.',
    '- Prefer the full automated OHOS loop when a local DevEco emulator or device is available: `${package.ohosRunCommand}`. Use `${package.ohosDeviceRunCommand}` for an already connected hdc target.',
    '- After changing shared Dart code, dependencies, `${package.examplePath}`, or platform-channel contracts, run existing-platform regression checks when the example has those platforms: `${package.androidRunCommand}` for Android and `${package.iosRunCommand}` for iOS. Add `--emulator <name>` to start a local emulator or simulator, or `--device <id>` to select a connected target. iOS builds use `--no-codesign` automatically.',
    '- Run `fluoh doctor --project --json --strict` when Android SDK, Xcode, DevEco Studio, signing tools, hdc, emulator, current project, selected SDK, fluoh installation, or source snapshot health is unclear; fix warnings before editing package logic.',
    '- Read JSON `nextCommand` and `diagnostics` before editing. Use the suggested next command and stable diagnostic code to decide whether to fix Dart analysis/tests, OHOS build, signing/permissions, hdc/emulator setup, install, launch, or runtime crash behavior.',
    ...
    '8. Verify existing platforms: after OHOS changes, follow the Platform Verification Matrix for each existing platform directory. Build-only evidence is not enough for release readiness; use `fluoh doctor --platform android --json --strict` or `fluoh doctor --platform ios --json --strict` to check native tooling, then use `${package.androidEmulatorRunCommand}` or `${package.iosSimulatorRunCommand}` so Android and iOS examples are launched and integration tests run when present. If a local Android, Xcode, device, or simulator environment is unavailable, record the exact skipped command and reason in the report.',
    ...
    '- `${package.androidRunCommand}`, `${package.iosRunCommand}`, run smoke checks, and integration tests pass when their platform directories and local toolchains exist; skipped Android/iOS checks include the exact command and environment blocker.',
...
List<String> _multiAdaptationCommandFlowLines() {
  return [
    ...
    '5. Existing-platform loop: for Android, run `fluoh doctor --platform android --json --strict`, then `fluoh run --platform android --package <name> --json` when `example/android` exists. For iOS, run `fluoh doctor --platform ios --json --strict`, then `fluoh run --platform ios --package <name> --json` when `example/ios` exists.',
...
List<String> _singleAdaptationCommandFlowLines(
  ...
  return [
    ...
    '5. Existing-platform loop: for Android, run `fluoh doctor --platform android --json --strict`, then `${package.androidEmulatorRunCommand}` when `${package.examplePath}/android` exists. For iOS, run `fluoh doctor --platform ios --json --strict`, then `${package.iosSimulatorRunCommand}` when `${package.examplePath}/ios` exists.',
    '6. Diagnostics loop: read `nextCommand`, `diagnostics[].code`, `stdoutTail`, `stderrTail`, and saved logs before editing. Fix `doctor` failures in local tooling, project warnings in repository configuration, and verification failures in the package or example that produced the diagnostic.',
...
List<String> _multiPlatformVerificationLines() {
  return [
    ...
    '- For Android/iOS run smoke checks, `fluoh run` launches the app, waits for Flutter launch output, captures output for `--log-duration`, sends `q`, saves the output under `\$FLUOH_HOME/package-runs`, and reports runtime failures through JSON diagnostics.',
...
List<String> _diagnosticsRoutingLines() {
  return [
    ...
    '- `android.devices_failed`, `android.emulators_failed`, `ios.devices_failed`, `ios.emulators_failed`: run `fluoh doctor --platform android --json --strict` or `fluoh doctor --platform ios --json --strict` and fix native target tooling before editing package logic.',
    '- `android.device_missing`, `ios.device_missing`: connect a matching target or rerun the platform run command so fluoh can start a local emulator/simulator.',
    '- `android.device_not_found`, `ios.device_not_found`: use an actual id from failed diagnostics or `fluoh devices --platform android` / `fluoh devices --platform ios`.',
    '- `android.device_ambiguous`, `ios.device_ambiguous`: pick one target and rerun with `--device <id>`.',
    '- `android.emulator_missing`, `ios.emulator_missing`: create a local Android emulator or iOS simulator before rerunning the platform run command.',
    ...
    '- `android.emulator_start_failed`, `ios.emulator_start_failed`: inspect native emulator launch output from `fluoh run` and repair the local emulator/simulator.',
    ...
    '- `ohos.device_missing`: start a DevEco emulator or connect a device, then rerun the OHOS run command or add `--device <id>`.',
...
List<String> _completionReportLines() {
  return [
    ...
    '- The report must include: package name, upstream version, FlutterOH SDK version, implementation summary, changed files, public API changes if any, permissions and OHOS config changes, commands run with exit results and relevant output tails, final `fluoh run --platform <platform> --json` outcome, a platform matrix for OHOS/Android/iOS with build, run, integration-test, device/simulator, and skip-blocker fields, signing mode, generated HAP paths, hilog path when present, remaining risks, and release recommendation.',
...
String packageImplementationGuideContent({
  ...
  return [
    ...
    '5. Run the full automated OHOS loop for each package example when a local DevEco emulator or device is available: `fluoh run --platform ohos --package <name> --json`. Add `--device <id>` for an already connected hdc target. Use `fluoh build --platform ohos --package <name> --auto-sign --json` as a build-only fallback when no device is available. JSON `nextCommand` and `diagnostics` give the next failure category.',
    '6. Run Android and iOS regression checks after shared or example changes when their platform directories exist: run `fluoh doctor --project --json --strict`, then use `fluoh run --platform android --package <name> --json` for Android and `fluoh run --platform ios --package <name> --json` for iOS, or add `--device <id>` for already connected targets. iOS builds use `--no-codesign`; document unavailable local Android/Xcode toolchains instead of guessing.',
    '7. Run `fluoh doctor --project --json --strict` when local Android SDK, Xcode, DevEco Studio, signing tools, hdc, emulator, current project, selected SDK, fluoh installation, or source snapshot state is unclear.',
    ...
    '8. Check existing platforms: follow the Platform Verification Matrix. Run `fluoh run --platform android --package <name> --json` when an Android example exists and `fluoh run --platform ios --package <name> --json` when an iOS example exists, or add `--device <id>` for already connected targets. Record exact skip reasons for unavailable local toolchains.',
...
String _singlePackageImplementationGuideContent({
  ...
  return [
    ...
    '5. Run the full automated OHOS loop when an example and local DevEco emulator or device are available: `${package.ohosRunCommand}`. Use `${package.ohosDeviceRunCommand}` for an already connected hdc target. Use `${package.ohosBuildCommand}` as a build-only fallback when no device is available. JSON `nextCommand` and `diagnostics` give the next failure category.',
    '6. Run Android and iOS regression checks after shared or example changes when their platform directories exist: run `fluoh doctor --project --json --strict`, then use `${package.androidEmulatorRunCommand}` for Android and `${package.iosSimulatorRunCommand}` for iOS, or add `--device <id>` for already connected targets. iOS builds use `--no-codesign`; document unavailable local Android/Xcode toolchains instead of guessing.',
    '7. Run `fluoh doctor --project --json --strict` when local Android SDK, Xcode, DevEco Studio, signing tools, hdc, emulator, current project, selected SDK, fluoh installation, or source snapshot state is unclear.',
    '8. Update `fluoh.yaml` and `FLUOH_CHANGELOG.md` when package version, upstream version, status, or release notes change.',
    ...
    '8. Check existing platforms: follow the Platform Verification Matrix. Run `${package.androidEmulatorRunCommand}` when an Android example exists and `${package.iosSimulatorRunCommand}` when an iOS example exists, or add `--device <id>` for already connected targets. Record exact skip reasons for unavailable local toolchains.',
    ...
    '- `${package.androidRunCommand}`, `${package.iosRunCommand}`, run smoke checks, and integration tests pass when those platforms exist and local toolchains are available; unavailable toolchains are documented with exact skipped commands and block a `ready` recommendation.',
