...
String packageAgentsInstructionsContent({
  ...
  return [
    ...
    '5. Add or update automated checks in the matching `fluoh_test/<package>/test` and `fluoh_test/<package>/example/test` directories. Keep them deterministic and runnable with `fluoh test run --package <name>`.',
    '6. Extend the matching `fluoh_test/<package>/example` app from the package\'s existing platforms plus OHOS, adding the smallest UI needed for manual verification when device behavior cannot be covered by tests.',
    ...
    '8. Update `FLUOH_CHANGELOG.md` for each package, run the matching `fluoh test run --package <name>`, review `git status --short --ignored=matching`, then commit before `fluoh package release --package <name>` or `fluoh package release --all`.',
    '',
    '## Before Commit',
    '',
...
String _singlePackageAgentsInstructionsContent({
  ...
  return [
    ...
    '3. Inspect the upstream Dart API and platform code, then implement OHOS behavior without changing public APIs unless upstream requires it.',
    '4. Run `fluoh deps get` after dependency, SDK, or package metadata changes.',
    '5. Add or update automated checks in `${package.testWorkspacePath}/test` and `${package.testWorkspacePath}/example/test`. Keep them deterministic and runnable with `${package.testRunCommand}`.',
    '6. Extend `${package.testWorkspacePath}/example` from the package\'s existing platforms plus OHOS, adding the smallest UI needed for manual verification when device behavior cannot be covered by tests.',
...
String packageImplementationGuideContent({
  ...
  return [
    ...
    '3. Keep package-specific `fluoh_test/<package>/test` and `fluoh_test/<package>/example/test` directories for automated checks, with `fluoh_test/<package>/example` apps for manual verification.',
    ...
    '6. Commit before `fluoh package sync`, `fluoh package release --package <name>`, or `fluoh package release --all`; release commands require a clean worktree.',
    '',
    '## Adaptation Workflow',
    '',
    ...
    '4. Add deterministic automated checks under the matching `fluoh_test/<package>/test` and `fluoh_test/<package>/example/test` directories.',
    '5. Extend the matching `fluoh_test/<package>/example` app from the package\'s existing platforms plus OHOS, and add focused manual verification actions when device behavior needs manual confirmation.',
    '6. Keep `packages.<name>.status: experimental` until that package is implemented, tested, and ready to be recommended.',
    '7. Run `fluoh deps get` after dependency or metadata changes, then run the matching `fluoh test run --package <name>`.',
    '8. Update `FLUOH_CHANGELOG.md`, commit, then use `fluoh package release --package <name>` or `fluoh package release --all` for release tagging.',
    '',
    '## Before Commit',
    '',
...
String _singlePackageImplementationGuideContent({
  ...
  return [
    ...
    '3. Keep `${package.testWorkspacePath}/test` and `${package.testWorkspacePath}/example/test` for automated checks, with `${package.testWorkspacePath}/example` for manual verification.',
    '4. Update `fluoh.yaml` and `FLUOH_CHANGELOG.md` when package version, upstream version, status, or release notes change.',
    '5. Run `${package.testRunCommand}` before release.',
    '6. Commit before `fluoh package sync` or `${package.releaseCommand}`; release commands require a clean worktree.',
    '',
    '## Adaptation Workflow',
    '',
    ...
    '4. Add deterministic automated checks under `${package.testWorkspacePath}/test` and `${package.testWorkspacePath}/example/test`.',
    '5. Extend `${package.testWorkspacePath}/example` from the package\'s existing platforms plus OHOS, and add focused manual verification actions when device behavior needs manual confirmation.',
    '6. Keep `packages.${package.name}.status: experimental` until the implementation is complete, tested, and ready to be recommended.',
    '7. Run `fluoh deps get` after dependency or metadata changes, then run `${package.testRunCommand}`.',
