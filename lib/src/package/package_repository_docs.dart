...
List<String> _completionReportLines() {
  return [
    ...
    '- The report must include: package name, upstream version, FlutterOH SDK version, implementation summary, changed files, public API changes if any, permissions and OHOS config changes, commands run with exit results and relevant output tails, final `fluoh run --platform <platform> --json` or build outcome, a platform matrix for OHOS/Android/iOS/macOS/Linux/Web/Windows with build, run, integration-test, device/simulator/host/browser, and skip-blocker fields, real `fluoh automate --json` evidence, automation coverage gates from dry-run or real automate JSON, functional interaction evidence for required scenarios, signing mode, generated HAP paths, hilog path when present, remaining risks, and release recommendation.',
