...
String packageAgentsInstructionsContent({
  ...
  return [
    ...
    '- Keep each package-specific `fluoh_test/<package>/test` and `fluoh_test/<package>/example/test` for automated verification. Use `fluoh_test/<package>/example` for manual platform verification when behavior needs a device.',
    ...
    '- Run `${packages.first.testRunCommand}` or another package-specific `fluoh test run --package <name>` before release. Commit before `fluoh package sync`, `fluoh package release --package <name>`, or `fluoh package release --all` because release commands require a clean worktree.',
    '',
    '## Adaptation Workflow',
    '',
    ...
    '5. From each matching `fluoh_test/<package>/example`, run `fluoh sdk use <sdk-version> --pub-get` if the IDE link is missing or stale; the generated example keeps its own `fluoh.yaml` pinned to the target SDK.',
    '6. Add or update automated checks in the matching `fluoh_test/<package>/test` and `fluoh_test/<package>/example/test` directories. Cover arguments, return shapes, errors, platform-channel names, and user-visible example flows where applicable.',
    '7. Extend the matching `fluoh_test/<package>/example` app from the package\'s existing platforms plus OHOS. Every important workflow should have a visible operation, expected result, pass/fail status, and failure hint.',
    ...
    '10. Update `FLUOH_CHANGELOG.md` for each package, run the matching `fluoh test run --package <name>`, review `git status --short --ignored=matching`, then commit before `fluoh package release --package <name>` or `fluoh package release --all`.',
    '',
    '## Before Commit',
    '',
...
String _singlePackageAgentsInstructionsContent({
  ...
  return [
    ...
    '- Keep tests focused on behavior and release contracts. For documentation or generated guidance, assert stable commands, files, schema keys, and deprecated terms rather than exact prose.',
    '- Run `${package.testRunCommand}` before release. Commit before `fluoh package sync` or `${package.releaseCommand}` because release commands require a clean worktree.',
    '',
    '## Adaptation Workflow',
    '',
    ...
    '3. Inspect the upstream Dart API and platform code, then implement OHOS behavior without changing public APIs unless upstream requires it.',
    '4. Run `fluoh deps get` after dependency, SDK, or package metadata changes.',
    '5. From `${package.testWorkspacePath}/example`, run `fluoh sdk use $sdkVersion --pub-get` if the IDE link is missing or stale; the generated example keeps its own `fluoh.yaml` pinned to `$sdkVersion`.',
    '6. Add or update automated checks in `${package.testWorkspacePath}/test` and `${package.testWorkspacePath}/example/test`. Cover arguments, return shapes, errors, platform-channel names, and user-visible example flows where applicable.',
    '7. Extend `${package.testWorkspacePath}/example` from the package\'s existing platforms plus OHOS. Every important workflow should have a visible operation, expected result, pass/fail status, and failure hint.',
    '8. Build the verification app with `fluoh flutter build hap --debug`; if it stops at signing, configure DevEco Studio debug signing and rerun. If it fails earlier, fix permissions, `reason`, `usedScene`, ArkTS, or package setup before release.',
    ...
    '10. Update `FLUOH_CHANGELOG.md`, run `${package.testRunCommand}`, review `git status --short --ignored=matching`, then commit before `${package.releaseCommand}`.',
    '',
    '## Before Commit',
    '',
    '- Review `git status --short --ignored=matching` and staged files before committing.',
...
String packageImplementationGuideContent({
  ...
  return [
    ...
    '3. Replace generated fluoh_test import smoke checks with package-specific contract tests and example actions before calling the package complete.',
    '4. Keep package-specific `fluoh_test/<package>/test` and `fluoh_test/<package>/example/test` directories for automated checks, with `fluoh_test/<package>/example` apps for manual verification.',
    ...
    '7. Run the matching `fluoh test run --package <name>` before release.',
    ...
    '4. Add deterministic automated checks under the matching `fluoh_test/<package>/test` and `fluoh_test/<package>/example/test` directories. Cover arguments, return shape, errors, and platform-channel names when applicable.',
    '5. From the matching `fluoh_test/<package>/example`, run `fluoh sdk use <sdk-version> --pub-get` when the IDE link is missing or stale.',
    '6. Extend the matching `fluoh_test/<package>/example` app from the package\'s existing platforms plus OHOS. Each important workflow should provide an operation, expected result, pass/fail status, and failure hint.',
    ...
    '9. Run `fluoh deps get` after dependency or metadata changes, then run the matching `fluoh test run --package <name>`.',
...
String _singlePackageImplementationGuideContent({
  ...
  return [
    ...
    '3. Replace generated fluoh_test import smoke checks with package-specific contract tests and example actions before calling the package complete.',
    '4. Keep `${package.testWorkspacePath}/test` and `${package.testWorkspacePath}/example/test` for automated checks, with `${package.testWorkspacePath}/example` for manual verification.',
    '5. Build `${package.testWorkspacePath}/example` with `fluoh flutter build hap --debug`; signing-only failures are local setup, earlier failures must be fixed.',
    '6. Update `fluoh.yaml` and `FLUOH_CHANGELOG.md` when package version, upstream version, status, or release notes change.',
    '7. Run `${package.testRunCommand}` before release.',
    ...
    '5. From `${package.testWorkspacePath}/example`, run `fluoh sdk use $sdkVersion --pub-get` when the IDE link is missing or stale.',
    ...
    '7. Run `fluoh flutter build hap --debug` from `${package.testWorkspacePath}/example`; fix permission, `reason`, `usedScene`, ArkTS, or package setup failures before release.',
    '8. Keep `packages.${package.name}.status: experimental` until the implementation is complete, tested, and ready to be recommended.',
    '9. Run `fluoh deps get` after dependency or metadata changes, then run `${package.testRunCommand}`.',
